...
// registerTools registers all kubectl commands as MCP tools
...
func buildToolOptions(cmd *cobra.Command) (string, []mcp.ToolOption, error) {
	// Build the command path (e.g., "get pods")
	var cmdPath []string
	current := cmd
	for current != nil && current.Name() != "kubectl" {
	...
	toolName := "kubectl_" + strings.Join(cmdPath, "_")
	...
	if description == "" {
		description = "Run the kubectl " + commandPath + " command"
	}
...
func createToolHandler(cmd *cobra.Command) server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		// Build arguments for the kubectl command
		args := cmdPath
		...
		return executeKubectlCommand(ctx, args)
	}
}

func executeKubectlCommand(ctx context.Context, args []string) (*mcp.CallToolResult, error) {
	// Create a context for output collection with a timeout
	...
	if err := cmd.Start(); err != nil {
		return mcp.NewToolResultError("Failed to start kubectl: " + err.Error()), nil
	}
